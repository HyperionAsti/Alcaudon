...
HyperionStraightLineAlgorithm::HyperionStraightLineAlgorithm(void)
{
...
void HyperionStraightLineAlgorithm::config_update(Config& config, uint32_t level)
{
  this->lock();
...
// HyperionStraightLineAlgorithm Public API
void HyperionStraightLineAlgorithm::pid_output(float &err, float &out, bool reset)
{
...
void HyperionStraightLineAlgorithm::pid_tunning(float &err, float &out, float &kp, float &ki, float &kd, bool reset)
{
  static float prev_mean_err = 0;
  static float integral = 0;
  static float derivative = 0;
