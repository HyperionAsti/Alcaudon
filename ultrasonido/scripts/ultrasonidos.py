...
from ultrasonido.msg import Distances
from std_msgs.msg import Bool
import time


#Definimos los pines que vamos a utilizar configurándolos como entrada o  salida segun sea el eco o el trigger  
...
# La variable stop nos sirve para parar los ultrasonidos si el nodo central lo ordena. Así evitamos un exceso de consumo
stop=0
def callback(data):
    global stop
    if data.data==1:
...
def ultrasonido_central():
    #Se inicia el nodo. Éste publicará las distancias obtenidas al nodo central y recibirá de éste la señal de stop cuando sea necesario.
    ...
    #Algoritmo del dispositivo de ultrasonido. La distancia de un objeto será proporcional al tiempo que permanezca encendido el pin del ECHO
   	if stop==0:
    ...
    #Si la señal de stop está habilitada, no se hace nada
	else:
		pass
